UPDATE Uk_accidenets.casualties
SET age_band_of_casualty = cslt.age_band_of_casualty,
    age_of_casualty = cslt.age_of_casualty,
    bus_or_coach_passenger = cslt.bus_or_coach_passenger,
    car_passenger = cslt.car_passenger,
    casualty_class = cslt.casualty_class,
    casualty_home_area_type = cslt.casualty_home_area_type,
    casualty_imd_decile = cslt.casualty_imd_decile,
    casualty_severity = cslt.casualty_severity,
    casualty_type = cslt.casualty_type,
    pedestrian_location = cslt.pedestrian_location,
    pedestrian_movement = cslt.pedestrian_movement,
    pedestrian_road_maintenance_worker = cslt.pedestrian_road_maintenance_worker,
    sex_of_casualty = cslt.sex_of_casualty
FROM
    (SELECT (CASE
                 ...
                 ELSE NULL
             END) age_band_of_casualty,
            (CASE
                 WHEN age_of_casualty = '-1' THEN NULL
             END) age_of_casualty,
            (CASE
            ...
            (CASE
                 ...
                 ELSE NULL
             END) pedestrian_road_maintenance_worker,
            (CASE
